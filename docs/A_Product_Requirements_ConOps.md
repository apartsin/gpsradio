...
- **Stories without OpenAI: a model on the phone.** Without a model the fallback could only read the source text as-is, and only when it was already in the listener's language (a Russian listener heard nothing from English Wikipedia). Now an on-device model retells the place's own facts as a short story, 3 to 4 sentences, in the session language. It uses only the facts it's given: no names, dates or numbers of its own. If it's missing, slow (45 s) or fails, the plain notes are read as before.
  - **Gemini Nano** (Android AICore via ML Kit GenAI Prompt API): built into supported phones (Pixel 9/10, Galaxy S25/S26, recent Xiaomi flagships). Nothing to download in the app; where Android still has to fetch it, Settings offers **Download Gemini Nano**.
  - **Open models run by Google's LiteRT-LM** (Apache-2.0, downloaded once from Hugging Face `litert-community`, runs on the CPU of any phone with enough memory): **Gemma 4 E2B** (2.6 GB, recommended, good Russian), **Qwen3 1.7B** (1 GB, for phones short of space or memory), **Gemma 4 E4B** (3.7 GB, best quality). The download resumes after a broken connection, checks free space and shows its progress; the model can be deleted to free the space.
